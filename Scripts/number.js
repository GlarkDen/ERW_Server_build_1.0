...
function TestAnswer (nid) {
    ...
    socket.onopen = function () {
        ...
        socket.send("Number: "+num+" "+nid+" "+document.getElementById(nid+'p').value+strlog);
    };
    ...
    socket.onmessage = function (event) {
        console.log("Получены данные " + event.data);
        if (document.getElementById(nid+'p').value==event.data) {
            if (time) {
                clearTimeout(timeColor);
                time = false;
            }
            document.getElementById(nid+'p').style="background-color: #4CAF50;";
        } else {
            document.getElementById(nid+'p').style="background-color: #f44336;";
            if (!time) {
    ...
    function White() {
        document.getElementById(nid+'p').style="background-color: #FFFFFF;";
        time = false;
    }
}
...
function Answer (nod) {
    ...
    socket.onmessage = function (event) {
        console.log("Получены данные " + event.data);
        document.getElementById(nod+'p').value=event.data;
        document.getElementById(nod+'p').style="background-color: #4CAF50;";
        socket.close();
    };
